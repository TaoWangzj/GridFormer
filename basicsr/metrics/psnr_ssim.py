...
def calculate_psnr_pt(img, img2, crop_border, test_y_channel=False, **kwargs):
    ...
    # img = torch.from_numpy(img / 255.).to(torch.float64).unsqueeze_(0)
    # img2 = torch.from_numpy(img2 / 255.).to(torch.float64).unsqueeze_(0)

    print(img.size())
    print(img2.size())

    img = img2tensor(img / 255., bgr2rgb=True, float32=True)
    img2 = img2tensor(img2 / 255., bgr2rgb=True, float32=True)
