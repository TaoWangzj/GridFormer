...
class ImageCleanModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        if load_path is not None:
            #load_path = load_path.replace(load_path.split("/")[-1], 'net_g_latest.pth')
    ...
    def optimize_parameters(self, current_iter):
        ...
        if self.cri_perceptual:
            ...
            l_style_value = 0.
            for pred in preds:
                l_percep, l_style = self.cri_perceptual(pred, self.gt)
                if l_percep is not None:
                    l_total += l_percep
                    l_percep_value += l_percep
                if l_style is not None:
                    l_total += l_style
                    loss_dict['l_style'] = l_style
            
            if l_percep_value !=0:
                loss_dict['l_percep'] = l_percep_value
            if l_style_value !=0:   
                loss_dict['l_style'] = l_style_value
    ...
    def pad2square_test(self, window_size):
        ...
        img = torch.zeros(1,3,X,X).type_as(self.lq) 
    ...
    def nondist_validation(self, dataloader, current_iter, tb_logger, save_img):
        ...
        if window_size:
            if window_size == 128:
                self.test = partial(self.pad2square_test, window_size)
            else:
                self.test = partial(self.pad_test, window_size)
        else:
    ...
    def save_network(self, net, net_label, current_iter,flag, param_key='params'):
        ...
        if flag:
            current_save = 'best'
            save_filename = f'{net_label}_{current_save}.pth'   ###### 需要修改的地方
        else:
            current_save = 'latest'
    ...
    def save_training_state(self, epoch, current_iter,flag):
        ...
        if current_iter != -1:
            state = {'epoch': epoch, 'iter': current_iter,'best_metric': self.best_metric, 'optimizers': [], 'schedulers': []}
